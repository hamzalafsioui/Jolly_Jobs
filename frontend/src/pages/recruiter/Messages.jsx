...
/* =============== component =================== */
export default function RecruiterMessages({ currentUser }) {
  ...
  useEffect(() => {
    ...
    // Don't leave channel on unmount => header needs it
  }, [currentUser]);
  ...
  function updateConvLastMsg(partnerId, msg) {
    setConversations((prev) =>
      prev.map((c) =>
        c.user.id === partnerId
  ...
  const handleSend = async () => {
    ...
    updateConvLastMsg(active.user.id, optimistic);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Communicate with your candidates.</p>
      </div>
